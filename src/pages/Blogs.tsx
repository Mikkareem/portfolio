...
import { Calendar, Hourglass, ArrowUpRight, ArrowRight } from 'lucide-react';
    ...
    <div id="blogs-page" className="relative min-h-screen pt-28 pb-20 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full z-10 relative space-y-16">
            ...
            <div className="flex p-1.5 rounded-xl bg-surface border border-white/5 shadow-md self-start md:self-end">
              {(['all', 'internal', 'medium'] as const).map(tab => (
                <button
                  ...
                  className={`px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all duration-300 ${activeTab === tab
                    ? 'bg-[#38bdf8] text-[#050816] glow-blue'
                    : 'text-[#94a3b8] hover:text-white'
                    }`}
                >
                  {tab === 'internal' ? 'Internal' : tab === 'medium' ? 'Medium' : 'All'}
                </button>
              ))}
            </div>
          </FadeIn>
        ...
        {filteredBlogs.length > 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {filteredBlogs.map((blog, idx) => (
                <StaggerItem key={blog.id}>
                  <div className={`glass-panel rounded-2xl overflow-hidden flex flex-col h-full hover:scale-[1.01] transition-all duration-300 group relative cursor-pointer border bg-gradient-to-br ${idx % 4 === 0
                    ...
                    {blog.isExternal ? (
                      <a
                        ...
                        aria-label={`Open ${blog.title} on Medium`}
                      />
                    ) : (
                      <Link
                    ...
                    <div className="h-56 overflow-hidden relative">
                      ...
                      <div className="absolute top-4 right-4 bg-black/70 backdrop-blur-md border border-white/10 px-3 py-1 rounded-full text-[10px] font-mono tracking-widest uppercase text-brand-accent font-semibold">
                        {blog.isExternal ? 'Medium' : 'Internal'}
                      </div>
                    </div>
                      ...
                      <div className="space-y-4 pt-4 border-t border-white/5">
                        ...
                        <div className="pt-2">
                          {blog.isExternal ? (
                            ...
                            >
                              Open In Medium <ArrowUpRight className="w-4 h-4 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
                            </span>
                          ) : (
                            <span
                              className="text-sm font-semibold text-white group-hover:text-brand-primary flex items-center gap-1.5 transition-colors"
          ...
          <EmptyState
            title="No data"
            description={`No articles found under the "${activeTab === 'internal' ? 'Internal' : activeTab === 'medium' ? 'Medium' : activeTab}" category.`}
            onReset={() => setActiveTab('all')}
            resetLabel="Show All Articles"
          />
        )}
