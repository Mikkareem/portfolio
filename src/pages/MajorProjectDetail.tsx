...
export const MajorProjectDetail = () => {
  const { slug } = useParams<{ slug: string }>();
  const project = portfolioRepository.getMajorProjects().find(p => p.slug === slug);
  ...
  const slides = project && hasScreenshots ? [project.thumbnailImage, ...project.screenshots].filter(Boolean) : [];
